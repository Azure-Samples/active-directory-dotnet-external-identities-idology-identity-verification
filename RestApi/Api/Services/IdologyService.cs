namespace Api.Services
{
    ...
    public class IdologyService : IIdologyService
    {
        ...
        public async Task<IExpectIdOutput> ExpectIdCall(IExpectIdInput expectIdInput)
        {
            ...
            using (var request = new HttpRequestMessage(HttpMethod.Post, "https://web.idologylive.com/api/idiq.svc"))
            {
                ...
                _logger.LogInformation($"Result: {responseData}");
            }
            ...
            if (summaryKey?.Value == "id.failure")
            {
                ...
                return new ExpectIdOutput {Success = false, Error = message.Value};
            }

            var doc = new XmlDocument();
            doc.LoadXml(responseData);

            return new ExpectIdOutput {Success = true};
        }
    }
}